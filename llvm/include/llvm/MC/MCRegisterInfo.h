...
private:
  ...
  const uint16_t *RegEncodingTable;           // Pointer to array of register
                                              // encodings.
  const uint16_t (*RegUnitIntervals)[2];      // Pointer to regunit interval table.
...
public:
  ...
  /// Returns the regunit interval [First, Last] for the given register.
  std::pair<unsigned, unsigned> getRegUnitInterval(MCRegister Reg) const {
    assert(hasRegUnitIntervals() &&
           "Target does not support regunit intervals");
    assert(Reg.id() < NumRegs && "Invalid register number");
    return {RegUnitIntervals[Reg.id()][0], RegUnitIntervals[Reg.id()][1]};
  }
};
